...
<div id="all">
	<table>
		<tr>
			...
			<th>申请日期:</th>
			<td><input type="text" value="${personApply.year }" readonly="readonly"></td>
		</tr>
		<tr>
			<th>性别:</th>
			<td><input type="text" value="${user.sex eq 0? '男':'女' }" readonly="readonly"></td>
			<th>荣誉称号:</th>
			<c:if test="${personApply.title eq 0}"><td><input type="text" value="全国劳模" readonly="readonly"></td></c:if>
			<c:if test="${personApply.title eq 1}"><td><input type="text" value="四川省劳模全国五一劳动奖章" readonly="readonly"></td></c:if>
			<c:if test="${personApply.title eq 2}"><td><input type="text" value="四川省五一劳动奖章" readonly="readonly"></td></c:if>
			<c:if test="${personApply.title eq 3}"><td><input type="text" value="其他荣誉称号" readonly="readonly"></td></c:if>
			<th>认定结果:</th>
			<c:if test="${personApply.result eq 0}"><td><input type="text" value="新上报" readonly="readonly"></td></c:if>
			<c:if test="${personApply.result eq 1}"><td><input type="text" value="已认定" readonly="readonly"></td></c:if>
			<c:if test="${personApply.result eq 2}"><td><input type="text" value="取消称号" readonly="readonly"></td></c:if>
		</tr>
		<tr>
			<th>民族:</th>
			...
			<c:if test="${user.unionName eq 2 }"><td><input type="text" value="省产业局" readonly="readonly"></td></c:if>
			<c:if test="${user.unionName eq 3 }"><td><input type="text" value="企业集团公会" readonly="readonly"></td></c:if>
			
			<th>身体状况:</th>
			<td><input type="text" value="${personApply.bodycondition }" readonly="readonly"></td>
		</tr>
		<tr>
			<th>籍贯:</th>
			...
			<th>家庭状况:</th>
			<td><input type="text" value="${personApply.difficulty }" readonly="readonly"></td>
		</tr>
		<tr>
			<th>权限:</th>
			...
			<th>就业情况:</th>
			<td><input type="text" value="${personApply.workcondition }" readonly="readonly"></td>
		</tr>
		<tr>
			<th>职位:</th>
			...
			<th>突出事迹:</th>
			<td><input type="text" value="${personApply.achievement }" readonly="readonly"></td>
		</tr>
		<tr>
			<th>状态:</th>
			...
			<th>地址:</th>
			<td colspan="3"><input type="text" value="${user.address }" readonly="readonly"></td>		
		</tr>
	</table>
</div>
</body>
